...
class PropositionalAction:
    ...
    def filt(self,inessentials):
        self.add_effects = [(cond,eff) for cond,eff in self.add_effects if not eff in inessentials]
        self.del_effects = [(cond,eff) for cond,eff in self.del_effects if not eff in inessentials]
        return self
